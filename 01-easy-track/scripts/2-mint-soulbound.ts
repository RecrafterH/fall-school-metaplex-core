...
async function main() {
  ...
  // Every Core asset lives at its own fresh address.
  const asset = generateSigner(umi);

  // PermanentFreezeDelegate makes the asset soulbound:
  //  - frozen: true      -> born frozen, so transfers and burns are rejected
  //  - authority: None   -> nobody can ever thaw it, so the freeze is permanent
