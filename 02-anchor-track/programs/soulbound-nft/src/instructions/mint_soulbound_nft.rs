...
///
/// The asset is made soul-bound by attaching the `PermanentFreezeDelegate`
/// plugin at creation time. `anchor test` checks the result.
pub fn handler(ctx: Context<MintSoulboundNft>, name: String, uri: String) -> Result<()> {
    ...
    CreateV2CpiBuilder::new(&mpl_core_program)
        ...
        //
        // PermanentFreezeDelegate makes the asset soul-bound:
        //  - frozen: true           -> born frozen, transfers and burns are rejected
        //  - PluginAuthority::None  -> nobody can ever thaw it, so it stays frozen
